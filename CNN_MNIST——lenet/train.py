...
def inference():
    x=tf.placeholder(tf.float32,[batch,pic_size,pic_size,channel],name='x')
    y=tf.placeholder(tf.float32,[batch,classes],name='y')
    ...
    with tf.variable_scope('conv1'):
        ...
        con1_b=tf.get_variable('bias',[con1deep],initializer=tf.constant_initializer(0.0))
        con11=tf.nn.conv2d(x,con1_w,strides=[1,1,1,1],padding='SAME')
        relu1=tf.nn.relu(tf.nn.bias_add(con11,con1_b))
    ...
    with tf.variable_scope('conv2'):
        ...
        con2_b=tf.get_variable('bias',[con2deep],initializer=tf.constant_initializer(0.0))
        con22=tf.nn.conv2d(pool1,con2_w,strides=[1,1,1,1],padding='SAME')
        relu2=tf.nn.relu(tf.nn.bias_add(con22,con2_b))
    ...
    with tf.Session() as sess:
        ...
        for e in range(8000):
            ...
            if (e+1)%1000==0:
                print('训练集第{}次，loss为{}，准确率为{}'.format(e + 1, loss_eval, acc_eval))
                ...
                print('测试集第{}次，loss为{}，准确率为{}'.format(e + 1, loss_eval, acc_eval))
